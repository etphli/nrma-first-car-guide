...
const bibliography = [
  ...
  ['NRMA Insurance', 'Comprehensive Car Insurance', 'https://www.nrma.com.au/car-insurance/comprehensive-car-insurance', 'Official product information and quote pathway used in the three-insurer comparison.'],
  ['Youi', 'Comprehensive Car Insurance', 'https://www.youi.com.au/car-insurance/comprehensive', 'Official product information, pricing factors and quote pathway.'],
  ['Budget Direct', 'Gold Comprehensive Car Insurance', 'https://www.budgetdirect.com.au/car-insurance/comprehensive-car-insurance.html', 'Official product information, optional extras and quote pathway.'],
...
function InsurancePage({ decision }) {
  ...
  const updateQuote = (provider, field, value) => setQuotes((current) => ({
    ...current,
    [provider]: { ...current[provider], [field]: value },
  }));
  const comparison = comprehensiveProviders.map((provider) => {
    const entry = quotes[provider.id] || { annual: '', excess: '' };
    return { ...provider, ...entry, annualValue: Number(entry.annual) || 0, excessValue: Number(entry.excess) || 0 };
  });
  const entered = comparison.filter((item) => item.annualValue > 0);
  const cheapest = entered.length >= 2 ? entered.reduce((best, item) => item.annualValue < best.annualValue ? item : best) : null;
  const lowestExcess = entered.filter((item) => item.excessValue > 0).sort((a, b) => a.excessValue - b.excessValue)[0];
  const highest = entered.length >= 2 ? Math.max(...entered.map((item) => item.annualValue)) : 0;
  return <main className="lb-learning-page"><div className="lb-wrap"><a className="lb-back-link" href="/"><Icon name="arrow" size={17} /> Back to Lowbeam</a><div className="lb-learning-hero hero-violet"><div className="lb-kicker"><i /> LEARNING JOURNEY / 04</div><h1>Cover,<br /><span>decoded.</span></h1><p>Start with what each insurance type protects. Then compare genuine Comprehensive quotes from three insurers on the same assumptions.</p></div><div className="lb-cover-tabs" role="tablist">{covers.map((item) => <button type="button" className={'cover-' + item.colour + (selected === item.id ? ' is-selected' : '')} role="tab" aria-selected={selected === item.id} key={item.id} onClick={() => setSelected(item.id)}><strong>{item.name}</strong><span>{item.summary}</span></button>)}</div><div className={'lb-cover-detail detail-' + cover.colour}><div><div className="lb-kicker"><i /> SELECTED COVER</div><h2>{cover.name}</h2><p>{cover.description}</p></div><div className="lb-cover-columns"><div><strong>Usually includes</strong>{cover.yes.map((item) => <span key={item}><Icon name="check" size={16} /> {item}</span>)}</div><div><strong>Usually does not include</strong>{cover.no.map((item) => <span key={item}><Icon name="close" size={16} /> {item}</span>)}</div></div></div><section className="lb-insurer-compare"><div className="lb-compare-heading"><div><span>REAL QUOTE COMPARISON</span><h2>Three insurers. One fair test.</h2></div><p>Lowbeam is not guessing insurer prices. Open each official quote tool, request Comprehensive cover using identical details, then enter the premium and basic excess you are shown.</p></div><div className="lb-quote-assumptions" aria-label="Details to use for every quote"><strong>Keep these details the same</strong><span>{decision.model || 'Your selected car'}</span><span>{decision.vehicleYear} model</span><span>{Number(decision.odometer).toLocaleString('en-AU')} km</span><span>Driver age {decision.driverAge}</span><span>{decision.licenceYears} {decision.licenceYears === 1 ? 'year' : 'years'} licensed</span><span>{decision.claimsLastFive} claims / 5 years</span><span>{decision.additionalDrivers === 'Yes' ? 'Additional drivers' : 'Sole driver'}</span><span>{decision.parking} parking</span><span>{Number(decision.kilometres).toLocaleString('en-AU')} km/year</span><span>{decision.usage} use</span><span>{decision.financeOwing === 'Yes' ? 'Finance owing' : decision.financeOwing === 'Not sure' ? 'Finance status unsure' : 'No finance owing'}</span><span>{decision.coverBasis}</span></div><div className="lb-provider-grid">{comparison.map((provider) => <article className={`lb-provider-card provider-${provider.colour}${cheapest?.id === provider.id ? ' is-cheapest' : ''}`} key={provider.id}><header><div><span>{cheapest?.id === provider.id ? 'LOWEST ENTERED PREMIUM' : 'COMPREHENSIVE'}</span><h3>{provider.name}</h3></div><a href={provider.url} target="_blank" rel="noreferrer">Get official quote <Icon name="up" size={16} /></a></header><p>{provider.detail}</p><div className="lb-provider-inputs"><label>Annual premium ($)<input type="number" inputMode="decimal" min="0" step="1" placeholder="Enter quote" value={provider.annual} onChange={(event) => updateQuote(provider.id, 'annual', event.target.value)} /></label><label>Basic excess ($)<input type="number" inputMode="decimal" min="0" step="1" placeholder="Enter excess" value={provider.excess} onChange={(event) => updateQuote(provider.id, 'excess', event.target.value)} /></label></div><dl><div><dt>Monthly equivalent</dt><dd>{provider.annualValue ? formatCurrency(provider.annualValue / 12) : '—'}</dd></div><div><dt>Premium + one excess</dt><dd>{provider.annualValue && provider.excessValue ? formatCurrency(provider.annualValue + provider.excessValue) : '—'}</dd></div></dl></article>)}</div><div className={`lb-comparison-result${cheapest ? ' is-ready' : ''}`} aria-live="polite"><div><span>LIVE COMPARISON</span>{cheapest ? <><h3>{cheapest.name} is lowest on the entered premium.</h3><p>{formatCurrency(cheapest.annualValue)} a year, or {formatCurrency(cheapest.annualValue / 12)} a month. That is {formatCurrency(highest - cheapest.annualValue)} less per year than the highest quote entered.</p></> : <><h3>Enter at least two official quotes.</h3><p>The result will appear here. Nothing is pre-filled because a real premium depends on the driver, car, address, use, claims history and policy choices.</p></>}</div><div className="lb-result-stat"><span>Lowest entered excess</span><strong>{lowestExcess ? `${lowestExcess.name} · ${formatCurrency(lowestExcess.excessValue)}` : 'Not enough data'}</strong></div></div><p className="lb-quote-disclaimer"><Icon name="shield" size={17} /> Quote prices are supplied by you from each insurer’s official quote journey. Lowbeam only converts annual amounts to monthly figures and compares the numbers entered; it does not provide or predict insurance quotes.</p></section><section className="lb-cover-tools"><div className="lb-compare-rules"><span>COMPARE LIKE WITH LIKE</span><h2>Do not let a cheaper number hide weaker settings.</h2><ol><li><b>01</b><p><strong>Choose Comprehensive</strong> at all three insurers.</p></li><li><b>02</b><p><strong>Use the same vehicle value</strong> where agreed or market value choices allow it.</p></li><li><b>03</b><p><strong>Match the basic excess</strong> as closely as each quote tool permits.</p></li><li><b>04</b><p><strong>Enter the same drivers, address, use and kilometres</strong> every time.</p></li></ol></div><div className="lb-cover-quiz"><span>QUICK COVER CHECK</span><h2>Your $14,000 car is stolen. Which option is designed to cover your own car?</h2>{[['ctp', 'CTP only'], ['third-party', 'Third Party Property only'], ['fire-theft', 'Third Party Fire & Theft'], ['comprehensive', 'Comprehensive']].map(([value, label]) => <button type="button" className={quizAnswer === value ? 'is-selected' : ''} onClick={() => setQuizAnswer(value)} key={value}>{label}</button>)}{quizAnswer && <p className={quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? 'is-correct' : 'is-wrong'} role="status"><strong>{quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? 'That can cover theft.' : 'Look again.'}</strong>{quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? ' Check limits, excess and policy wording before choosing.' : ' CTP covers injuries, while Third Party Property focuses on damage you cause to other property.'}</p>}</div></section><div className="lb-premium-note"><div><div className="lb-kicker"><i /> FOUR THINGS TO COMPARE</div><h2>Price is only one line.</h2></div><p>Read the premium, excess, exclusions and whether the insurer uses market value or agreed value. Ask who can drive the car and what happens after a claim.</p></div><JourneyNext number="✓" eyebrow="Journey complete" title="Take your buyer brief with you." copy="You have checked the common mistakes, real costs, purchase protections and cover types. Use the saved answers when comparing real cars." href="/" button="Return to your car check" /></div></main>;
}
...
function LowbeamPage({ pathname }) {
  ...
  if (pathname === '/insurance') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><InsurancePage decision={decision} /><LowbeamFooter isHome={false} /></div>;
