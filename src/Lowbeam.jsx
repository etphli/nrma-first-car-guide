...
import { mistakes, researchSources, SourceNote, RightsExplained, PremiumExplained, BudgetExample } from './AssignmentContent.jsx';
import QuotePreparation from './QuotePreparation.jsx';
import QuoteComparison from './QuoteComparison.jsx';
...
const DEFAULT_DECISION = {
  ...
  financeOwing: 'No',
  coverBasis: 'Agreed value where available',
...
const covers = [
  { id: 'ctp', name: 'CTP / Green Slip', summary: 'Required cover', colour: 'lime', description: 'Compulsory Third Party insurance helps cover injuries to people caused by a vehicle crash. It is required for registration, but it does not cover damage to cars or property.', yes: ['Injuries to people under scheme rules', 'Registration requirement', 'State scheme rules'], no: ['Damage to your car', 'Theft or weather damage', 'Damage to another car'] },
  { id: 'third-party', name: 'Third Party Property Damage', summary: 'Cover what you damage', colour: 'coral', description: 'This cover helps pay for damage you cause to someone else’s vehicle or property. Damage to your own car is generally excluded, although some policies have limited uninsured-driver benefits.', yes: ['Damage to another person’s property', 'Legal liability within the policy', 'A lower-cost option for some older cars'], no: ['Repairs to your own car', 'Theft of your car', 'Storm or hail damage'] },
...
const comprehensiveProviders = [
  {
    id: 'nrma',
    name: 'NRMA Insurance',
    url: 'https://www.nrma.com.au/car-insurance/comprehensive-car-insurance',
    colour: 'lime',
    detail: 'Use standard Comprehensive so the cover level is comparable.',
  },
  {
    id: 'youi',
    name: 'Youi',
    url: 'https://www.youi.com.au/car-insurance/comprehensive',
    colour: 'blue',
    detail: 'Roadside assistance is included; check its limits when comparing value.',
  },
  {
    id: 'budget',
    name: 'Budget Direct',
    url: 'https://www.budgetdirect.com.au/car-insurance/comprehensive-car-insurance.html',
    colour: 'coral',
    detail: 'Roadside assistance is separate; check whether an online discount applies.',
  },
];

const bibliography = researchSources;
...
function SignalRing({ score, large = false }) {
  ...
  return <div className={'lb-ring' + (large ? ' lb-ring-large' : '')} style={{ '--score-angle': angle + 'deg' }} aria-label={'Illustrative planning score ' + score + ' out of 100'}><div><strong>{score}</strong><span>CLASSROOM MODEL</span></div></div>;
}
...
function calculateDecision(decision) {
  ...
  const vehicleAge = Math.max(0, new Date().getFullYear() - Number(decision.vehicleYear));
...
function Hero({ decision, result, onOpenLab }) {
  return <section className="lb-hero"><div className="lb-wrap lb-hero-grid"><div className="lb-hero-copy"><div className="lb-kicker"><i /> YOUR FIRST-CAR FIELD GUIDE</div><h1>Buy your first car with your eyes open.</h1><p>Welcome to Lowbeam. Learn the ten first-car mistakes, test your budget and know your rights before you hand over the money.</p><div className="lb-hero-actions"><button className="lb-button lb-button-lime" type="button" onClick={onOpenLab}>Check a car <Icon name="arrow" size={18} /></button><a className="lb-text-link" href="#library">Explore the guide <Icon name="arrow" size={17} /></a></div><div className="lb-current-readout"><span>Example or saved running-cost estimate</span><strong>{decision.model || 'First car'} · {formatCurrency(result.monthlyCost)}/month</strong></div><nav className="lb-hero-shortcuts" aria-label="Popular starting points"><button type="button" onClick={onOpenLab}><Icon name="wallet" size={18} /><span><small>Run the numbers</small>Budget check</span><Icon name="arrow" size={16} /></button><a href="/mistakes"><Icon name="warning" size={18} /><span><small>Learn the traps</small>Mistake map</span><Icon name="arrow" size={16} /></a><a href="/protect"><Icon name="shield" size={18} /><span><small>Before you pay</small>Buyer checklist</span><Icon name="arrow" size={16} /></a></nav></div><CarStudio /></div></section>;
}
...
function QuickCheck({ decision, setDecision, result, onToast }) {
  ...
  const steps = [
    ...
    <div className="lb-wizard-fields" key="driver"><Field label="Driver age"><input type="number" min="16" max="99" value={decision.driverAge} onChange={(event) => update('driverAge', Number(event.target.value))} /></Field><Field label="Years licensed"><input type="number" min="0" max="80" value={decision.licenceYears} onChange={(event) => update('licenceYears', Number(event.target.value))} /></Field><Field label="Claims in the last 5 years"><input type="number" min="0" max="10" value={decision.claimsLastFive} onChange={(event) => update('claimsLastFive', Number(event.target.value))} /></Field><Field label="Any additional drivers?"><select value={decision.additionalDrivers} onChange={(event) => update('additionalDrivers', event.target.value)}><option>No</option><option>Yes</option></select></Field><Field label="Is finance owing?"><select value={decision.financeOwing} onChange={(event) => update('financeOwing', event.target.value)}><option>No</option><option>Yes</option><option>Not sure</option></select></Field><Field label="Vehicle value preference"><select value={decision.coverBasis} onChange={(event) => update('coverBasis', event.target.value)}><option>Agreed value where available</option><option>Market value</option><option>Compare both</option></select></Field><Field label="Safety rating"><select value={decision.safety} onChange={(event) => update('safety', event.target.value)}><option value="Not checked">Not checked</option><option value="5-star">5-star</option><option value="4-star">4-star</option><option value="3-star">3-star or lower</option></select></Field><Field label="Main use"><select value={decision.usage} onChange={(event) => update('usage', event.target.value)}><option>Mixed</option><option>School or work</option><option>Weekend trips</option><option>Long commute</option></select></Field></div>,
    <div className="lb-wizard-checks" key="checks"><button type="button" className={decision.history ? 'is-selected' : ''} onClick={() => update('history', !decision.history)}><Icon name="history" size={20} /><span><strong>Vehicle history checked</strong><small>Finance, stolen and write-off status</small></span></button><button type="button" className={decision.inspected ? 'is-selected' : ''} onClick={() => update('inspected', !decision.inspected)}><Icon name="wrench" size={20} /><span><strong>Inspection completed and report reviewed</strong><small>A mechanic checks what the ad cannot show</small></span></button></div>,
  ];
  ...
  const next = () => { if (step < steps.length - 1) setStep((current) => current + 1); else { const blob = new Blob([buildBrief(decision, result)], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'Lowbeam-buyer-brief.txt'; link.click(); URL.revokeObjectURL(url); onToast('Buyer brief downloaded'); } };
  return <section className="lb-quick-check" id="lab"><div className="lb-wrap lb-quick-grid"><div className="lb-wizard"><div className="lb-step-line"><span>Step {step + 1} of {steps.length}</span><div style={{ gridTemplateColumns: `repeat(${steps.length}, 1fr)` }}>{steps.map((_, item) => <i className={item <= step ? 'is-active' : ''} key={item} />)}</div></div><h2>{headings[step]}</h2><p className="lb-wizard-help">Start with the example, then replace it with your details. Running costs are editable on the Costs page.</p>{steps[step]}<div className="lb-wizard-actions">{step > 0 && <button type="button" className="lb-back-button" onClick={() => setStep((current) => current - 1)}>Back</button>}<button type="button" className="lb-button lb-button-blue" onClick={next}>{step === steps.length - 1 ? 'Download my buyer brief' : 'Next question'} <Icon name="arrow" size={18} /></button></div></div><aside className="lb-check-summary"><span>Your live check</span><SignalRing score={result.score} large /><dl><div><dt>Car</dt><dd>{decision.model || carType}</dd></div><div><dt>Age + odometer</dt><dd>{result.vehicleAge} yrs · {Math.round(decision.odometer / 1000)}k km</dd></div><div><dt>Monthly reality</dt><dd>{formatCurrency(result.monthlyCost)}</dd></div><div><dt>Income left</dt><dd className={result.disposableLeft < 0 ? 'is-negative' : ''}>{formatCurrency(result.disposableLeft)}</dd></div><div><dt>Safety</dt><dd>{decision.safety}</dd></div></dl><p>This is a made-up classroom planning score, not an NRMA rating, safety test or approval to buy.</p><details className="lb-score-method"><summary>How is it calculated?</summary><p>Budget contributes 45%, an age and usage formula 22%, and checks, selected safety stars and seller type 33%. These weights are design choices, not research findings. An old five-star rating is not equivalent to a current one. The price and driver profile do not change running costs automatically.</p><a href="/costs">Edit the actual running-cost assumptions →</a></details></aside></div></section>;
}
...
function Library({ onToast }) {
  const [saved, setSaved] = useStoredState('lowbeam-library-v1', false);
...
function BibliographyPage() {
  return <main className="lb-bibliography"><div className="lb-wrap"><a className="lb-back-link" href="/"><Icon name="arrow" size={17} /> Back to Lowbeam</a><div className="lb-biblio-hero"><div className="lb-kicker"><i /> RESEARCH TRAIL</div><h1>Bibliography<span>.</span></h1><p>The sources behind the learning library and the checks in the lab. All links open in a new tab.</p></div><div className="lb-biblio-list">{bibliography.map((item, index) => <a href={item[2]} target="_blank" rel="noreferrer" className="lb-biblio-row" key={item[2]}><span className="lb-biblio-number">{String(index + 1).padStart(2, '0')}</span><span><strong>{item[0]}</strong><b>{item[1]}</b><small>{item[3]}</small></span><Icon name="up" size={19} /></a>)}</div><div className="lb-biblio-notes"><div><span>Visual credit</span><p>The interactive road car is adapted from the BMW M4 Competition M Package model by SRT Performance, credited in the original project as CC BY 4.0. The source page could not be independently rechecked during this review; confirm the licence before public reuse. Lowbeam removes the supplied livery and adds original materials, lighting and interaction. The sports car is an illustration, not a first-car recommendation. Icons and the mistake-road graphic are inline SVG artwork. The still hatchback image was supplied with the original project; its external source is not recorded. The older /og demo uses NRMA Insurance imagery and fonts from nrma.com.au; it is not the assignment homepage. The NRMA motoring organisation and NRMA Insurance are separate businesses; neither endorses this project.</p></div><div><span>AI and authorship note</span><p>This is an original student product concept. AI assisted with prototyping and copy development; consumer and budgeting guidance was reviewed against the linked primary sources on 5 September 2026. Example budgets and the planning score are classroom illustrations.</p></div><div><span>Accessed</span><p>5 September 2026</p></div></div></div></main>;
}
...
function MistakeMap() {
  ...
  return <div className="lb-map-layout">
    <section ref={stageRef} className="lb-map-stage" aria-label={`Mistake ${active + 1} of ${mistakes.length}`}>
      <div className="lb-map-heading"><h1>The top 10<br />mistakes<span>.</span></h1><p>Choose a stop. Know what to avoid.</p><nav className="assignment-stops" aria-label="Jump to a mistake">{mistakes.map((mistake, index) => <button type="button" key={mistake[0]} aria-label={`Mistake ${index + 1}: ${mistake[0]}`} aria-pressed={active === index} onClick={() => goToStop(index)}>{String(index + 1).padStart(2, '0')}</button>)}</nav><strong aria-live="polite"><b>{active + 1}</b> of {mistakes.length}</strong><div role="progressbar" aria-label="Mistake map progress" aria-valuemin="1" aria-valuemax={mistakes.length} aria-valuenow={active + 1}><i style={{ width: `${((active + 1) / mistakes.length) * 100}%` }} /></div><small>{active === mistakes.length - 1 ? 'You made it through' : 'Keep scrolling'}</small></div>
...
function ProtectPage() {
  ...
  const checks = ['Match the VIN with the paperwork', 'Run a PPSR vehicle history check', 'Review an independent inspection report', 'Read the contract before paying'];
  ...
  return <main className="lb-learning-page"><div className="lb-wrap"><a className="lb-back-link" href="/"><Icon name="arrow" size={17} /> Back to Lowbeam</a><div className="lb-learning-hero hero-blue"><div className="lb-kicker"><i /> LEARNING JOURNEY / 03</div><h1>Protect<br /><span>yourself.</span></h1><p>A careful buyer is not being difficult. Check the car, the seller and the paperwork before money changes hands.</p></div><section className="lb-protection-intro"><div><h2>Who are you buying from?</h2><p>The seller changes which protections may apply. Select one to see the practical difference.</p></div><div className="lb-seller-grid">{[['Dealer', 'shield', 'Australian Consumer Law guarantees apply to consumer purchases from licensed dealers. The remedy depends on the fault and circumstances.'], ['Private seller', 'person', 'Protections can be more limited. Confirm ownership, run a history check and do not skip the inspection.']].map(([name, icon, copy]) => <button type="button" aria-pressed={sellerType === name} className={sellerType === name ? 'is-selected' : ''} onClick={() => setSellerType(name)} key={name}><div className="lb-round-icon"><Icon name={icon} size={26} /></div><h2>{name}</h2><p>{copy}</p><span>{sellerType === name ? 'Selected' : 'Choose this seller'} <Icon name="arrow" size={15} /></span></button>)}</div></section><RightsExplained sellerType={sellerType} /><div className="lb-check-card"><div><div className="lb-kicker"><i /> READY-TO-BUY CHECKLIST</div><h2>Make every check visible.</h2><p>{checked.length}/4 complete. {checked.length === 4 ? 'You have covered the core checks.' : 'Do not let excitement hide the gaps.'}</p><div className="lb-check-progress"><i style={{ width: `${checked.length / checks.length * 100}%` }} /></div></div><div>{checks.map((item, index) => <label key={item} className={checked.includes(index) ? 'is-checked' : ''}><input type="checkbox" checked={checked.includes(index)} onChange={() => setChecked((list) => list.includes(index) ? list.filter((value) => value !== index) : [...list, index])} /><span><Icon name="check" size={16} /></span>{item}</label>)}</div></div><section className="lb-scenario"><div><span>DEAL-BREAKER DRILL</span><h2>The seller says another buyer is waiting and refuses an inspection. What do you do?</h2></div><div className="lb-scenario-options">{[['deposit', 'Pay a small deposit to hold it'], ['promise', 'Accept a verbal promise that it is fine'], ['walk', 'Pause or walk away until it can be inspected']].map(([value, label]) => <button type="button" className={scenario === value ? 'is-selected' : ''} onClick={() => setScenario(value)} key={value}>{label}</button>)}</div>{scenarioCopy && <div className={'lb-scenario-result' + (scenario === 'walk' ? ' is-correct' : '')} role="status"><Icon name={scenario === 'walk' ? 'check' : 'warning'} size={22} /><p><strong>{scenarioCopy[0]}</strong>{scenarioCopy[1]}</p></div>}</section><div className="lb-rights-note"><strong>Keep a paper trail.</strong><p>Save the listing, inspection report, messages, receipt, contract and proof of payment. Written records make later questions easier to answer.</p></div><JourneyNext number="04" eyebrow="Final checkpoint" title="Choose the cover." copy="You have checked the car and seller. Now compare what each insurance type protects and what it leaves with you." href="/insurance" button="Open Cover, decoded" /></div></main>;
}

function InsurancePage({ decision, setDecision }) {
  const [selected, setSelected] = useState('comprehensive');
  const [quotes, setQuotes] = useStoredState('lowbeam-insurer-quotes-v1', {
    nrma: { annual: '', excess: '' },
    youi: { annual: '', excess: '' },
    budget: { annual: '', excess: '' },
  });
  const [quizAnswer, setQuizAnswer] = useState('');
  const cover = covers.find((item) => item.id === selected) || covers[3];
  return <main className="lb-learning-page"><div className="lb-wrap"><a className="lb-back-link" href="/"><Icon name="arrow" size={17} /> Back to Lowbeam</a><div className="lb-learning-hero hero-violet"><div className="lb-kicker"><i /> LEARNING JOURNEY / 04</div><h1>Cover,<br /><span>decoded.</span></h1><p>Start with what each insurance type protects. Then compare genuine Comprehensive quotes from three insurers on the same assumptions.</p></div><nav className="lb-page-jumps" aria-label="Insurance sections"><a href="#cover-types">Understand cover</a><a href="#compare-quotes">Compare quotes ↓</a><a href="#prepare-quotes">Prepare your details</a></nav><div className="lb-cover-tabs" id="cover-types" role="tablist">{covers.map((item) => <button type="button" className={'cover-' + item.colour + (selected === item.id ? ' is-selected' : '')} role="tab" aria-selected={selected === item.id} key={item.id} onClick={() => setSelected(item.id)}><strong>{item.name}</strong><span>{item.summary}</span></button>)}</div><div className={'lb-cover-detail detail-' + cover.colour}><div><div className="lb-kicker"><i /> SELECTED COVER</div><h2>{cover.name}</h2><p>{cover.description}</p></div><div className="lb-cover-columns"><div><strong>Usually includes</strong>{cover.yes.map((item) => <span key={item}><Icon name="check" size={16} /> {item}</span>)}</div><div><strong>Usually does not include</strong>{cover.no.map((item) => <span key={item}><Icon name="close" size={16} /> {item}</span>)}</div></div></div><PremiumExplained /><section className="lb-insurer-compare" id="compare-quotes"><div className="lb-compare-heading"><div><span>REAL QUOTE COMPARISON</span><h2>Three insurers. One fair test.</h2></div><p>Get prices from the insurers, then enter them below. Or try the clearly labelled example prices to practise comparing cover.</p></div><div className="lb-quote-assumptions" aria-label="Details to use for every quote"><strong>Keep these details the same</strong><span>{decision.model || 'Your selected car'}</span><span>{decision.vehicleYear} model</span><span>{Number(decision.odometer).toLocaleString('en-AU')} km</span><span>Driver age {decision.driverAge}</span><span>{decision.licenceYears} {decision.licenceYears === 1 ? 'year' : 'years'} licensed</span><span>{decision.claimsLastFive} claims / 5 years</span><span>{decision.additionalDrivers === 'Yes' ? 'Additional drivers' : 'Sole driver'}</span><span>{decision.parking} parking</span><span>{Number(decision.kilometres).toLocaleString('en-AU')} km/year</span><span>{decision.usage} use</span><span>{decision.financeOwing === 'Yes' ? 'Finance owing' : decision.financeOwing === 'Not sure' ? 'Finance status unsure' : 'No finance owing'}</span><span>{decision.coverBasis}</span></div><QuoteComparison providers={comprehensiveProviders} quotes={quotes} setQuotes={setQuotes} /><details className="lb-quote-preparation-details" id="prepare-quotes"><summary><span>Prepare your details for the insurer</span><small>Car, drivers and policy choices</small></summary><QuotePreparation decision={decision} setDecision={setDecision} /></details></section><section className="lb-cover-tools"><div className="lb-compare-rules"><span>COMPARE LIKE WITH LIKE</span><h2>Do not let a cheaper number hide weaker settings.</h2><ol><li><b>01</b><p><strong>Choose Comprehensive</strong> at all three insurers.</p></li><li><b>02</b><p><strong>Use the same vehicle value</strong> where agreed or market value choices allow it.</p></li><li><b>03</b><p><strong>Match the basic excess</strong> as closely as each quote tool permits.</p></li><li><b>04</b><p><strong>Enter the same drivers, address, use and kilometres</strong> every time.</p></li></ol></div><div className="lb-cover-quiz"><span>QUICK COVER CHECK</span><h2>Your $14,000 car is stolen. Which option is designed to cover your own car?</h2>{[['ctp', 'CTP only'], ['third-party', 'Third Party Property only'], ['fire-theft', 'Third Party Fire & Theft'], ['comprehensive', 'Comprehensive']].map(([value, label]) => <button type="button" className={quizAnswer === value ? 'is-selected' : ''} onClick={() => setQuizAnswer(value)} key={value}>{label}</button>)}{quizAnswer && <p className={quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? 'is-correct' : 'is-wrong'} role="status"><strong>{quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? 'That can cover theft.' : 'Look again.'}</strong>{quizAnswer === 'fire-theft' || quizAnswer === 'comprehensive' ? ' Check limits, excess and policy wording before choosing.' : ' CTP covers injuries, while Third Party Property focuses on damage you cause to other property.'}</p>}</div></section><div className="lb-premium-note"><div><div className="lb-kicker"><i /> FOUR THINGS TO COMPARE</div><h2>Price is only one line.</h2></div><p>Read the premium, excess, exclusions and whether the insurer uses market value or agreed value. Ask who can drive the car and what happens after a claim.</p></div><JourneyNext number="✓" eyebrow="Journey complete" title="Take your buyer brief with you." copy="You have checked the common mistakes, real costs, purchase protections and cover types. Use the saved answers when comparing real cars." href="/" button="Return to your car check" /></div></main>;
}
...
function LowbeamHome({ onContact }) {
  ...
  return <div className="lowbeam-site"><LowbeamHeader onContact={onContact} /><Hero decision={decision} result={result} onOpenLab={openLab} /><TaskRail /><QuickCheck decision={decision} setDecision={setDecision} result={result} onToast={showToast} /><HowItWorks /><Library onToast={showToast} /><Mission completed={completed} setCompleted={setCompleted} onToast={showToast} /><LowbeamFooter /><CommandPalette />{toast && <div className="lb-toast" role="status"><Icon name="check" size={17} />{toast}</div>}</div>;
}

function LowbeamPage({ pathname }) {
  const [decision, setDecision] = useStoredState('lowbeam-decision-v1', DEFAULT_DECISION);
  const result = useMemo(() => calculateDecision(decision), [decision]);
  if (pathname === '/bibliography') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><BibliographyPage /><LowbeamFooter isHome={false} /><CommandPalette /></div>;
  if (pathname === '/mistakes') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><MistakePage /><LowbeamFooter isHome={false} /><CommandPalette /></div>;
  if (pathname === '/costs') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><CostsPage decision={decision} setDecision={setDecision} result={result} /><LowbeamFooter isHome={false} /><CommandPalette /></div>;
  if (pathname === '/protect') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><ProtectPage /><LowbeamFooter isHome={false} /><CommandPalette /></div>;
  if (pathname === '/insurance') return <div className="lowbeam-site"><LowbeamHeader isHome={false} /><InsurancePage decision={decision} setDecision={setDecision} /><LowbeamFooter isHome={false} /><CommandPalette /></div>;
...
export default function Lowbeam() {
  ...
  useEffect(() => {
    document.title = ({ '/bibliography': 'Sources', '/mistakes': 'Top 10 mistakes', '/costs': 'Buying smart', '/protect': 'Protect yourself', '/insurance': 'Insurance explained' }[pathname] || 'Your first-car guide') + ' · Lowbeam';
